...
local QRNNRNN, parent = torch.class('onmt.QRNNRNN', 'nn.AbstractRecurrent')

function QRNNRNN:__init(outputSize, poolingMethod, rho)
...
--  else
  ...
  elseif self.poolingMethod == 'ifo' then  numGates = 4
    ...
    print('QRNNRNN: Invalid poolingMethod: ' .. self.poolingMethod)
    os.exit()
  end
...
function QRNNRNN:getHiddenState(step, input)
...
function QRNNRNN:setHiddenState(step, hiddenState)
   step = step == nil and (self.step - 1) or (step < 0) and (self.step - step - 1) or step
...
------------------------- forward backward -----------------------------
function QRNNRNN:updateOutput(input)
   local prevOutput, prevCell = unpack(self:getHiddenState(self.step-1, input))

   -- output(t), cell(t) = QRNNRNN{input(t), output(t-1), cell(t-1)}
   local output, cell
...
function QRNNRNN:getGradHiddenState(step)
...
function QRNNRNN:setGradHiddenState(step, gradHiddenState)
   local _step = self.updateGradInputStep or self.step
   step = step == nil and (_step - 1) or (step < 0) and (_step - step - 1) or step
...
function QRNNRNN:_updateGradInput(input, gradOutput)
...
function QRNNRNN:_accGradParameters(input, gradOutput, scale)
